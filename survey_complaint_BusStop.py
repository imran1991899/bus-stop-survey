...
st.markdown("""
    ...
    /* Action Buttons Colors for Photos */
...
# --------- Logic Functions (Google Integration) ---------
...
if staff_id: st.info(f"**{staff_dict[staff_id]}**")
...
# --------- REVERTED PHOTO SECTION ---------
st.subheader("📸 Evidence (3 Photos Required)")

# Only show inputs if fewer than 3 photos are taken
if len(st.session_state.photos) < 3:
    st.markdown(f"**Capturing Photo {len(st.session_state.photos) + 1} of 3**")
    cam_in = st.camera_input("Take Photo", key=f"capture_cam_{len(st.session_state.photos)}")
    file_in = st.file_uploader("Or Upload File", type=["jpg","png","jpeg"], key=f"capture_file_{len(st.session_state.photos)}")
...
# Display Captured Photos in a 3-Column Grid
if st.session_state.photos:
    ...
    for i, p in enumerate(st.session_state.photos):
        with photo_grid[i]:
            ...
            # Retake clears the whole list for that slot to restart
...
if st.button("Submit Survey"):
    if not staff_id or not stop or len(st.session_state.photos) < 3 or None in st.session_state.responses.values():
        st.error("Please ensure all questions are answered and exactly 3 photos are captured.")
    else:
        with st.spinner("Submitting..."):
            ...
            row = [timestamp, staff_id, staff_dict[staff_id], current_depot, current_route, stop] + \
                  [st.session_state.responses[q] for q in all_questions] + ["; ".join(photo_urls)]
            header = ["Timestamp", "Staff ID", "Name", "Depot", "Route", "Stop"] + all_questions + ["Photos"]
            
            gs_id = find_or_create_gsheet("survey_responses", FOLDER_ID)
            append_row(gs_id, row, header)
