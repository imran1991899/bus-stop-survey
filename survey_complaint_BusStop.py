...
def get_authenticated_service():
    ...
    if "oauth_flow" not in st.session_state:
        flow = Flow.from_client_secrets_file(
            CLIENT_SECRETS_FILE,
            scopes=SCOPES,
            redirect_uri='https://bus-stop-survey-7xwztn3gd99rsrshahjki5.streamlit.app/'  # Your actual redirect URI here
        )
    ...
    if "code" in query_params:
        ...
        try:
            ...
            base_url = 'https://bus-stop-survey-7xwztn3gd99rsrshahjki5.streamlit.app/'
...
if len(st.session_state.photos) < 5:
    photo = st.camera_input(
        f"📷 Take Photo #{len(st.session_state.photos) + 1}"
    )
    if photo:
